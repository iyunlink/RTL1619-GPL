...
static inline int is_clk_regmap(struct clk_hw *hw)
{
	if (!hw->init || !hw->init->ops)
		return 0;

	return is_clk_regmap_ops(hw->init->ops);
}

static inline int is_clk_det(struct clk_hw *hw)
{
	if (!hw->init || !hw->init->ops)
		return 0;
	return is_clk_det_ops(hw->init->ops);
}
