...
/* Version Information */
#define DRIVER_VERSION "v2.14.DHC (2020/09/24)"
...
{
	u32 rx_max_size = tp->netdev->mtu + VLAN_ETH_HLEN + ETH_FCS_LEN;

	return (ALIGN(rx_max_size, 1024) + 6 * 1024);
}
...
static inline u32 fc_pause_off_auto(struct r8152 *tp)
{
	...
	return (ALIGN(rx_max_size, 1024) + 14 * 1024);
}
...
static void r8156_fc_parameter(struct r8152 *tp)
{
	...
	case RTL_VER_10:
	case RTL_VER_11:
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0a6, pause_on / 8);
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0aa, pause_off / 8);
		break;
	case RTL_VER_12:
	case RTL_VER_13:
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0a6, pause_on / 16);
		ocp_write_word(tp, MCU_TYPE_PLA, 0xc0aa, pause_off / 16);
		break;
	default:
		break;
	}
...
static void rtl8156_up(struct r8152 *tp)
{
	...
	rx_max_size = tp->netdev->mtu + VLAN_ETH_HLEN + ETH_FCS_LEN;
			 ...
			 const struct usb_device_id *id)
{
	...
	tp->rx_pending = 50 * RTL8152_MAX_RX;
	tp->fc_pause_on = 12288;
	tp->fc_pause_off = 16384;
