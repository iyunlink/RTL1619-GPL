...
#ifdef CONFIG_R8125_ACP
        int node = pdev->dev.parent ? dev_to_node(pdev->dev.parent) : -1;
#endif
        int rc;
...
#ifdef CONFIG_R8125_ACP
        tp->tally_vaddr = kzalloc_node(sizeof(*tp->tally_vaddr), GFP_KERNEL, node);
        tp->tally_paddr = virt_to_phys(tp->tally_vaddr);
#else
...
static int rtl8125_alloc_tx_desc(struct rtl8125_private *tp)
{
        ...
        for (i = 0; i < tp->num_tx_rings; i++) {
                ring = &tp->tx_ring[i];
#ifdef CONFIG_R8125_ACP
                ring->TxDescArray = kzalloc_node(R8125_TX_RING_BYTES, GFP_KERNEL, node);
                ring->TxPhyAddr = virt_to_phys(ring->TxDescArray);
#else
...
static int rtl8125_alloc_rx_desc(struct rtl8125_private *tp)
{
        ...
        for (i = 0; i < tp->num_rx_rings; i++) {
                ring = &tp->rx_ring[i];
#ifdef CONFIG_R8125_ACP
                ring->RxDescArray = kzalloc_node(tp->RxDescRingLength, GFP_KERNEL, node);
                ring->RxPhyAddr = virt_to_phys(ring->RxDescArray);
#else
