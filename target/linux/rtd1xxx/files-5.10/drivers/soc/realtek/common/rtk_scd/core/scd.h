...
typedef struct {
	unsigned long val;   // protocol
} scd_protocol;

typedef struct {
	unsigned int val;   // protocol
} scd_protocol32;
...
static inline void scd_set_drvdata(scd_device *device, void *data)
{
...
typedef struct
{
	...
	int (*enable)(scd_device *dev, unsigned char on_off);
	int (*set_param)(scd_device *dev, unsigned long param_id, unsigned long val);
	int (*get_param)(scd_device *dev, unsigned long param_id, unsigned long *p_val);

	//-- add these to avoid confusing in SCControl(GetParam/SetParm)
	int (*set_protocol)(scd_device *dev, unsigned int val);
	int (*get_protocol)(scd_device *dev, unsigned int *p_val);

	//---------
	int (*activate)(scd_device *dev);
	int (*deactivate)(scd_device *dev);
	int (*reset)(scd_device *dev);
	int (*get_atr)(scd_device *dev, scd_atr *p_atr);
	int (*get_card_status)(scd_device *dev);
	int (*poll_card_status)(scd_device *dev);
	int (*xmit)(scd_device *dev, unsigned char *p_data, unsigned int len);
	int (*read)(scd_device *dev, unsigned char *p_data, unsigned int len);
