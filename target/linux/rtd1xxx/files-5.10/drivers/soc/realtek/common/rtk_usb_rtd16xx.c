...
static int type_c_plug_config(struct rtk_usb *rtk_usb, int dr_mode, int cc)
{
	...
	if (wait_count < 0) {
		pr_err("wait u3phy clock fail. drd=0x%x u3host=0x%x\n",
	...
	if (cc == disable_cc) {
		...
		/* disable u3phy clock */
		writel(~BIT(1) & readl(drd_u3phy_pipe_clock),
			    drd_u3phy_pipe_clock);
		writel(~BIT(1) & readl(u3host_u3phy_pipe_clock),
			    u3host_u3phy_pipe_clock);

		if (cc == enable_cc1) {
			writel(~BIT(1) & readl(drd_tc_mux_ctrl),
				    drd_tc_mux_ctrl);
			pr_info("%s: enable_cc1 drd u3phy for CHIP_ID_RTD1619\n",
				    __func__);
		} else if (cc == enable_cc2) {
			writel(BIT(1) | readl(drd_tc_mux_ctrl),
				    drd_tc_mux_ctrl);
			pr_info("%s: enable_cc2 drd u3phy for CHIP_ID_RTD1619\n",
				    __func__);
		}
		/* enable u3phy clock */
		writel(BIT(1) | readl(drd_u3phy_pipe_clock),
...
EXPORT_SYMBOL(rtk_usb_rtd16xx_init);
