...
extern int rtk_rpc_mem_int(void);
...
#else
#define RPC_INTR_VE3_DEV_TOTAL 2
#endif

#ifdef CONFIG_REALTEK_RPC_HIFI
typedef enum {
	RPC_INTR_DEV_SH_ID0, /* intr HIFI write */
	RPC_INTR_DEV_HS_ID1, /*intr HIFI read */
	RPC_INTR_HIFI_DEV_TOTAL
} NUM_INTR_HIFI_DEV;
#else
#define RPC_INTR_HIFI_DEV_TOTAL 2
#endif
...
#else
#define RPC_NR_KERN_VE3_DEVS 2
#endif

#ifdef CONFIG_REALTEK_RPC_HIFI
typedef enum {
	RPC_KERN_DEV_SH_ID0, /* kern HIFI write */
	RPC_KERN_DEV_HS_ID1, /* kern HIFI read */
	RPC_NR_KERN_HIFI_DEVS   /* total ring buffer number */
} NUM_KERN_HIFI_DEV;
#else
#define RPC_NR_KERN_HIFI_DEVS 2
#endif
...
#define RPC_POLL_DEV_ADDR (AVCPU_NOCACHE + 0x040FF000)
...
#define RPC_KERN_DEV_ADDR (AVCPU_NOCACHE + 0x04100200)
...
#define RPC_POLL_RECORD_ADDR (AVCPU_NOCACHE + 0x04100000)
...
#ifdef CONFIG_REALTEK_RPC_HIFI
#ifndef RPC_INTR_HIFI_RECORD_ADDR
#define RPC_INTR_HIFI_RECORD_ADDR (AVCPU_NOCACHE + 0x04102400)
#endif /* RPC_INTR_HIFI_RECORD_ADDR */
#ifndef RPC_INTR_HIFI_DEV_ADDR
#define RPC_INTR_HIFI_DEV_ADDR (AVCPU_NOCACHE + 0x04101a00)
#endif /* RPC_INTR_HIFI_DEV_ADDR */

#ifndef RPC_KERN_HIFI_RECORD_ADDR
#define RPC_KERN_HIFI_RECORD_ADDR (AVCPU_NOCACHE + 0x04102700)
#endif /* RPC_KERN_HIFI_RECORD_ADDR */
#ifndef RPC_KERN_HIFI_DEV_ADDR
#define RPC_KERN_HIFI_DEV_ADDR (AVCPU_NOCACHE + 0x04102000)
#endif /* RPC_KERN_HIFI_DEV_ADDR */
#endif
...
typedef struct RPC_HIFI_DEV { /*size should be 64 bytes*/
	uint32_t ringBuf;
	uint32_t ringStart;
	uint32_t ringEnd;
	volatile uint32_t ringIn;
...
extern RPC_DEV_EXTRA rpc_intr_extra[RPC_NR_DEVS/RPC_NR_PAIR + RPC_INTR_VE3_DEV_TOTAL + RPC_INTR_HIFI_DEV_TOTAL];
#elif defined(CONFIG_REALTEK_RPC_VE3)
extern RPC_DEV_EXTRA rpc_intr_extra[RPC_NR_DEVS/RPC_NR_PAIR + RPC_INTR_VE3_DEV_TOTAL];
#else
...
static inline int get_ring_data_size(RPC_DEV_EXTRA *extra)
{
	int ret;
	...
	if (extra->serialno > 5) {
		ret = __get_ring_data_size(hifi_dev->ringStart, hifi_dev->ringEnd,
			hifi_dev->ringIn, hifi_dev->ringOut);
	} else {
		ret = __get_ring_data_size(dev->ringStart, dev->ringEnd,
...
static inline uint32_t get_ring_data(const char *func, RPC_DEV_EXTRA *extra,
		uint32_t out, char *buf, int datasize)
{
	RPC_DEV *dev = extra->dev;
	uint32_t ringStart, ringEnd, ringIn;
#ifdef CONFIG_REALTEK_RPC_HIFI
	RPC_HIFI_DEV *hifi_dev = extra->hifi_dev;
	...
	ringIn = dev->ringIn;
#endif
	int size = __get_ring_data_size(ringStart, ringEnd,
			ringIn, out);
	int tail = ringEnd - out;
	...
	if (tail >= datasize) {
		...
		my_memcpy((int *)(buf + tail), (int *)AVCPU2SCPU(ringStart),
				datasize - tail);
#else
		memcpy(buf, AVCPU2SCPU(out), tail);
		memcpy(buf + tail, AVCPU2SCPU(ringStart), datasize - tail);
#endif
...
static inline void dump_ring_buffer(const char *func, RPC_DEV_EXTRA *extra)
{
	int i;
	RPC_DEV *dev = extra->dev;
	uint32_t ringStart, ringEnd, ringIn, ringOut;
#ifdef CONFIG_REALTEK_RPC_HIFI
	RPC_HIFI_DEV *hifi_dev = extra->hifi_dev;
	...
	ringOut = dev->ringOut;
#endif
	char *bufaddr = AVCPU2SCPU(ringStart);
	pr_err("==*== %s Start:%p Out:%p next:%p In:%p End:%p size:%d %s ==*==\n",
			func, AVCPU2SCPU(ringStart),
			AVCPU2SCPU(ringOut),
			AVCPU2SCPU(extra->nextRpc),
			AVCPU2SCPU(ringIn),
			AVCPU2SCPU(ringEnd),
			get_ring_data_size(extra),
			in_atomic() ? "atomic" : "");
...
static inline void peek_rpc_struct(const char *func, RPC_DEV_EXTRA *extra, loff_t num)
{
	RPC_STRUCT rpc;
	uint32_t pid;
	uint32_t arg;
...
#ifdef CONFIG_REALTEK_RPC_HIFI
		if (extra->serialno > 5)
			out = extra->hifi_dev->ringOut;
		else
			out = extra->dev->ringOut;
#else
		out = extra->dev->ringOut;
#endif

	out = get_ring_data(func, extra, out, (char *)&rpc, sizeof(RPC_STRUCT));
	if (out == 0)
		return;
	...
	if (rpc.programID == AUDIO_AGENT && rpc.versionID == 0) {
		...
		out = get_ring_data(func, extra, out, (char *)&arg, sizeof(uint32_t));
		if (out == 0)
			return;
		arg = ntohl(arg);
		...
		out = get_ring_data(func, extra, out, (char *)&pid, sizeof(uint32_t));
		if (out == 0)
			return;
		pid = ntohl(pid);
	...
	if (rpc.programID == REPLYID && rpc.versionID == REPLYID)
		trace_rtk_rpc_peek_rpc_reply((struct rpc_struct_tp *)&rpc,
				(u32)refclk_get_counter(refclk), num, pid, false);
	else
		trace_rtk_rpc_peek_rpc_request((struct rpc_struct_tp *)&rpc,
				(u32)refclk_get_counter(refclk), num, rpc.taskID, false);
}
...
#ifdef CONFIG_REALTEK_RPC_HIFI
	if (extra->serialno > 5)
		ret = (extra->hifi_dev->ringOut == extra->nextRpc);
	else
		ret = (extra->dev->ringOut == extra->nextRpc);
#else
	ret = (extra->dev->ringOut == extra->nextRpc);
#endif
...
static inline int ring_empty(RPC_DEV_EXTRA *extra)
{
	int ret = 0;

#ifdef CONFIG_REALTEK_RPC_HIFI
	if (extra->serialno > 5)
		ret = (extra->hifi_dev->ringOut == extra->hifi_dev->ringIn);
	else
		ret = (extra->dev->ringOut == extra->dev->ringIn);
#else
	ret = (extra->dev->ringOut == extra->dev->ringIn);
#endif
...
#define need_dispatch(extra) (extra->currProc == NULL && !ring_empty(extra) && rpc_done(extra))
#endif
		...
		uint32_t taskID)
{
	...
	task = get_pid_task(find_pid_ns(taskID, &init_pid_ns), PIDTYPE_PID);
	if (task == NULL) {
...
#endif
#ifdef CONFIG_REALTEK_RPC_HIFI
extern volatile RPC_HIFI_DEV *rpc_intr_hifi_devices;
extern volatile RPC_HIFI_DEV *rpc_kern_hifi_devices;
#endif
