...
static struct sb2_sem *mcp_lock = NULL;
...
int mcp_do_command(mcp_desc* p_desc, int n_desc)
{
	...
	if (mcp_lock)
		sb2_sem_lock_interruptible(mcp_lock, SB2_SEM_NO_WARNING | SB2_SEM_TIMEOUT_INFINITY);
...
static int mcp_probe(struct platform_device *pdev)
{
	...
	mcp_lock = of_sb2_sem_get(pdev->dev.of_node, 0);
	if (IS_ERR_OR_NULL(mcp_lock)) {
		pr_info("%s: no hw semaphore for mcp_lock\n", __func__);
		mcp_lock = NULL;
	}
