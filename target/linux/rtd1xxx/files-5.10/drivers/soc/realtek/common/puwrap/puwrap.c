...
int pu_alloc_dma_buffer(unsigned int size, unsigned long *phys_addr, unsigned long *base, unsigned int mem_type)
{
    ...
    do {
        ...
        dmabuf = ion_alloc(size, heap, flags);
...
EXPORT_SYMBOL(pu_alloc_dma_buffer);
...
EXPORT_SYMBOL(pu_free_dma_buffer);
...
EXPORT_SYMBOL(pu_mmap_dma_buffer);
...
unsigned long pu_mmap_kernel_buffer(unsigned long phys_addr, unsigned int size)
{
    ...
    do {
        ...
        if(dmabuf != NULL)
        {
            ...
            if (ret) {
                printk(KERN_ERR "[PUWRAP] dma_buf_begin_cpu_access error ret=%ld (size=%d) \n", ret, size);
                break;
            }
            virt_addr = dma_buf_vmap(dmabuf);
            if (IS_ERR_OR_NULL(virt_addr)) {
                printk(KERN_ERR "[PUWRAP] dma_buf_vmap error %p (size=%d) \n", virt_addr, size);
                break;
            }
...
EXPORT_SYMBOL(pu_mmap_kernel_buffer);
...
void pu_unmap_kernel_buffer(unsigned long base, unsigned long phys_addr)
{
    ...
    do {
        ...
        if(dmabuf != NULL)
        {
            dma_buf_vunmap(dmabuf, (void *)base);
...
EXPORT_SYMBOL(pu_unmap_kernel_buffer);
...
EXPORT_SYMBOL(pu_set_dovi_flag);
...
static int pu_probe(struct platform_device *pdev)
{
    ...
    if (pu_kobj) {
        ret = sysfs_create_group(pu_kobj, &rtk_pu_attr_group);
        if (ret)
