...
#define RTK_IR_HW1 "rtk-ir-hw1"
#define HW1_BASE	0x80

static struct rtk_ir_hw_decoder *rtk_ir_hw1_decoders = {
#ifdef CONFIG_IR_RTK_HW1_NEC
	&rtk_ir_nec,
#endif
#ifdef CONFIG_IR_RTK_HW1_RSTEP
	&rtk_ir_rstep,
#endif
#ifdef CONFIG_IR_RTK_HW1_XMP
	&rtk_ir_xmp,
#endif
#ifdef CONFIG_IR_RTK_HW1_SKY
	&rtk_ir_sky,
#endif
};

static void rtk_ir_hw1_init(struct rtk_ir *ir)
{
	struct rtk_ir_hw_decoder *dec = ir->hw1.dec;
	struct rc_dev *rc = ir->hw1.rc;
	void __iomem *base = ir->base + HW1_BASE;
	...
	if (rc->rc_map.rc_proto == RC_PROTO_XMP) {
		...
		writel(0x11d89385, base + IRCOMCAST1_TIMING);
	} else {
...
void rtk_ir_isr_hw1(struct rtk_ir *ir)
{
	...
	if(dec->scancode(&request, data)) {
		if (request.repeat)
			...
			rc_keydown(ir->hw1.rc, request.protocol, request.scancode,
				   request.toggle);
	}
}
...
int rtk_ir_hw1_probe(struct rtk_ir *ir)
{
	...
	rc->priv = ir;
	rc->device_name = RTK_IR_HW1;
	rc->input_phys = RTK_IR_HW1 "/input0";
	rc->dev.parent = dev;
	rc->driver_name = RTK_IR_HW1;
	rc->timeout = MS_TO_US(10);

	map_name = of_get_property(dn, "hw1,rc-map-name", NULL);
	...
	ir->hw1.rc = rc;
	ir->hw1.dec = rtk_ir_hw1_decoders;
