...
static int rtk_usb_phy_write(struct rtk_usb_phy_s *rtk_phy,
	    char addr, char data)
{
	volatile unsigned int regVal;
	struct reg_addr *regAddr = rtk_phy->reg_addr;
...
void rtk_rle0599_phy_toggle(struct usb_phy *usb2_phy, bool isConnect)
{
	...
	if (!phy_data) {
		pr_err("%s %s ERROR! phy_data is NULL", __FILE__, __func__);
		return;
	}
...
static int rtk_rle0599_set_parameter_open(struct inode *inode,
	    struct file *file)
{
	return single_open(file, rtk_rle0599_set_parameter_show,
		    inode->i_private);
}
...
static int rtk_rle0599_toggle_show(struct seq_file *s, void *unused)
{
	...
	seq_puts(s, "ehco 1 to toggle Page1 addr 0xe0 BIT(2)\n");
...
static inline void create_debug_files(struct rtk_usb_phy_s *rtk_phy)
{
	...
	if (!debugfs_create_file("parameter", S_IRUGO,
...
static int rtk_usb_rle0599_phy_probe(struct platform_device *pdev)
{
	...
	if (dev->of_node) {
		...
		dev_dbg(dev, "%s %s REG_WRAP_VStatusOut2=%p\n", __FILE__,
			    __func__, addr->REG_WRAP_VStatusOut2);
	...
	if (dev->of_node) {
		...
		tmp_addr = kzalloc(sizeof(char) * phy_data_page0_size, GFP_KERNEL);
		tmp_data = kzalloc(sizeof(char) * phy_data_page0_size, GFP_KERNEL);

		ret = of_property_read_u8_array(dev->of_node,
			    "phy_data_page0_addr", tmp_addr,
			    phy_data_page0_size);
		if (ret)
			goto err;
		ret = of_property_read_u8_array(dev->of_node,
			    "phy_data_page0_data", tmp_data,
			    phy_data_page0_size);
		if (ret)
			goto err;
		...
		ret = of_property_read_u8_array(dev->of_node,
			    "phy_data_page1_addr", tmp_addr,
			    phy_data_page1_size);
		if (ret)
			goto err;
		ret = of_property_read_u8_array(dev->of_node,
			    "phy_data_page1_data", tmp_data,
			    phy_data_page1_size);
		if (ret)
			goto err;
