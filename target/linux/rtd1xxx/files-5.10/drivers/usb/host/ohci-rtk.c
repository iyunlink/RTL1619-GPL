...
static int ohci_rtk_setup(struct usb_hcd *hcd)
{
	...
	ohci_dbg(ohci, "%s, ohci:%p", __func__, ohci);
...
static void ohci_rtk_probe_work(struct work_struct *work)
{
	...
	rtk_usb_init_port_power_on_off(dev, true);
...
static int ohci_rtk_drv_probe(struct platform_device *pdev)
{
	...
	hcd = usb_create_hcd(&ohci_rtk_dmp_hc_driver, &pdev->dev,
		    dev_name(&pdev->dev));
	if (!hcd)
		return -ENOMEM;
	...
	if (ohci) {
		...
		if (resuming) {
			ohci_dbg(ohci, "%s wait for resuming done !!\n",
				    __func__);
			if (!wait_for_completion_timeout(&ohci->resuming_done,
