...
dl_pack/xz=xz -zc -7e $(DL_XZ_OPTS) > $(1)
...
define DownloadMethod/gits
	echo "Checking out files from multiple git repositories..."; \
	rm -rf $(TMP_DIR)/$(SUBDIR) && \
	$(foreach URL,$(URLS), \
		$(call wrap_mirror,$(1),$(2), \
			$(call DownloadMethod/rawgit) \
		) \
		$(call UNPACK_CMD,$(TMP_DIR)/dl) && \
		rm $(DL_DIR)/$(FILE); \
	) \
	cd $(TMP_DIR) && \
	export TAR_TIMESTAMP= && \
	$(call dl_tar_pack,$(DL_DIR)/$(FILE),$(SUBDIR)) && \
	rm -rf $(SUBDIR);
endef
...
define DownloadMethod/repo
	$(call wrap_mirror,$(1),$(2), \
		echo "Checking out files from the repo repository..."; \
		mkdir -p $(TMP_DIR)/dl && \
		cd $(TMP_DIR)/dl && \
		rm -rf $(SUBDIR) && mkdir -p $(SUBDIR) && \
		(cd $(SUBDIR) &&  \
			$(TOPDIR)/tools/repo init -u $(URL) $(REPO_OPTS) && \
			$(TOPDIR)/tools/repo sync && \
			$(TOPDIR)/tools/repo forall -c 'echo Path:$$$${REPO_PATH} Project:$$$${REPO_PROJECT} Rev:$$$${REPO_RREV} Cur:$$$${REPO_LREV}' > REVISION \
		) && \
		export TAR_TIMESTAMP=`cd $(SUBDIR)/.repo/manifests && git log -1 --format='@%ct'` && \
		find $(SUBDIR) -name .git | xargs rm -rf && \
		find $(SUBDIR) -name .repo | xargs rm -rf && \
		echo "Packing checkout..." && \
		$(call dl_tar_pack,$(TMP_DIR)/dl/$(FILE),$(SUBDIR)) && \
		mv $(TMP_DIR)/dl/$(FILE) $(DL_DIR)/ && \
		rm -rf $(SUBDIR); \
...
define Download/default
  ...
  $(if $(PKG_SOURCE_URL_PATHS),URLS:=$(addprefix $(PKG_SOURCE_URL)/,$(PKG_SOURCE_URL_PATHS)))
  ...
  OPTS:=$(if $(PKG_SOURCE_BRANCH),--branch $(PKG_SOURCE_BRANCH))$(if $(PKG_GIT_OPT), $(PKG_GIT_OPT))
  REPO_OPTS:=$(if $(PKG_SOURCE_BRANCH),-b $(PKG_SOURCE_BRANCH))$(if $(PKG_MANIFEST_NAME), -m $(PKG_MANIFEST_NAME))
endef

define Download/rtk
  ifneq (,$(findstring $(PROTO),git gits repo))
  URL:=$(subst @DHCGERRIT,ssh://dhcgerrit,$(URL))
  $(if $(URLS),URLS:=$(subst @DHCGERRIT,ssh://dhcgerrit,$(URLS)))
  endif
endef
